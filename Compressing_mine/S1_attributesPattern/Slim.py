...
class Slim(BaseEstimator, TransformerMixin):
    ...
    def original_codeTable(self, X, y=None):
        X = check_array(X, accept_sparse=['csr'])
        self.totalFeatures_ = X.shape[1]
        (totalElements, nextFeatureId, featureDescById, exampleIdsByfeatureId,
         featureIdByExampleIds) = self.loadData(X, y)
        ...
        # 初始遍历所有的item（singleton），计算standard code table（ST） length && database length
    ...
    ### fit function—————————————————————————————————————————————————————————————————————————————————————————————————————————————————
    # Input:Database X
    # Procedure: 产生Candidate list, 组成新Pattern, 更新筛选直到满足终止条件
    # 无output 不停更新自身属性
    ###——————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————
    def fit(self, X, y=None):
        X = check_array(X, accept_sparse=['csr'])
        self.totalFeatures_ = X.shape[1]
        (totalElements, nextFeatureId, featureDescById, exampleIdsByfeatureId, featureIdByExampleIds) = self.loadData(X,
                                                                                                                      y)
        ...
        # 初始遍历所有的item（singleton），计算standar code table（ST） length && database length
        for (featureId, exampleIds) in exampleIdsByfeatureId.items():
            feature = Feature(featureId, 1, set([featureId]))
            feature.exampleIds = exampleIds
            feature.originalExampleIds = exampleIds
        ...
        # 每更新一次都要重新对比计算Gain，产生新的候选集
        while len(self.candidates_) > 0:
            # 取出candidate list的第一个
            ...
            for candidate1 in self.candidates_:
                # 更新了以后查看要合并到一个pattern中的两个元素 feature1和feature2的变化
                # situation1: 如果两者有一个不出现了，直接舍弃掉
                if candidate1.i_feature.support == 0 or candidate1.j_feature.support == 0:
                    continue

                # situation2: 检查是否为重复pattern
    ...
    def transform(self, X):
        ...
        if X.shape[1] != self.totalFeatures_:
            raise ValueError(
                "matrix to transform should have the same features as fit matrix: " + str(self.totalFeatures_))
        ...
        # 扫描每一行，code table中的每一个feature
        # 如果codetable中feature是这一行的子集，
        # 则这一行的feature set与featureId的components取差集，
        # f在该feature中加入行号，出现次数+1
    ...
    def do_transform(self, codeTable=None):
        ...
        # print(unusedFeatureIds)

        for f in ct:
            # 替换每一个feature（Pattern）在每一行的element 为 最新的featureId
    ...
    def get_code_table2(self):
        codeTable = self.codeTable_
        if codeTable is None:
            raise (ValueError("'fit' or 'fit_transform' must first be called to generate the code table"))

        return list((f.featureId, f.support, f.components, f.exampleIds) for f in codeTable)

    # 通过featureId得到真实的feature
    ...
    # loadData (4 functions)
    def loadData(self, X, y=None, returnXref=True):
        if y is not None:
            y = check_array(y, accept_sparse=True, ensure_2d=False, allow_nd=True, ensure_min_samples=0,
    ...
    # ———————————————————————————————————————————————————————————————————————————————————————————————————
    #####   Function：loadDataSparse, loadDataDense
    # Input：Dataset
    # Procedure: 遍历每行的每个元素
    # Parameters(Dicts): featureDesc( key: (element, 1), value: featureId(按照元素出现顺序编号) )
    #                    featureIdByDesc( key: featureId, value:(element, 1))
    #                    exampleIdsByfeatureId( key: featureId, value: rowId(元素出现的行数))
    #                    featureIdByexampleIds( key: rowId, value: featureId )
    # Output: totalElements, nextFeatureId, exampleIdsByfeatureId, featureIdByExampleIds
    # ——————————————————————————————————————————————————————————————————————————————————————————————————
            ...
            for (i, idx) in enumerate(row.indices):
                ...
                if element is not None and element != self.na_value_:
                    ...
                    if featureId is None:
                        ...
                        # print('featureIdByDesc = ', featureIdByDesc)

                    # Dict exampleIdsByfeatureId --> key: featureId; value: rowId, 行号
                    # 利用set 追加字典数据 value 无序
                    exampleIdsByfeatureId.setdefault(featureId, set()).add(rowId)

                    # Dict featureIdByExampleIds --> key:rowId 行号; key:featureId 元素对应编号（出现顺序）
    ...
    # 初始化 featureDesById,featureIdByDes--> featureId, id; featureDesc, code?
    ...
    def generateCandidates(self, i_feature, i=None):
        # 初始化时 i = null, 从头遍历code Table，两两Pattern对比
        ...
        for j in range(startAt, len(self.codeTable_)):
            ...
            # i -> 用于对比的element; j -> code table已存在的element
            # 查看两者是否有交集
    ...
    def addNewFeature(self, candidate):
        ...
        if candidate.i_feature.originalExampleIds is None or candidate.j_feature.originalExampleIds is None:
            ...
            f.totalSupport = len(
                candidate.i_feature.originalExampleIds.intersection(candidate.j_feature.originalExampleIds))
    ...
    def decompress(self, transformed=None, code_table=None, feature_cross_reference=None):
        if transformed is None:
            if code_table is not None:
                warnings.warn(
                    "When 'transformed' is unspecified, the internal code table is used. 'code_table' is ignored.",
                    RuntimeWarning)
            if feature_cross_reference is not None:
                warnings.warn(
                    "When 'transformed' is unspecified, the internal cross reference is used. 'feature_cross_reference' is ignored.",
                    RuntimeWarning)
...
##computeLengths: 计算新pattern的编码长度和替换后在database中的长度=support * codeLength
...
class Candidate:
    ...
    def computeStatistics(self, totalElements, force=False):
        ...
        (self.standardCodeLength, self.databaseLength) = self.cl.computeLengths(self.support, self.cardinality,
                                                                                self.new_totalElements)
